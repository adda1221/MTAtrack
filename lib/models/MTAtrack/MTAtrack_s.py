...
class MTAtrackS(nn.Module):
    ...
    def __init__(self, backbone, transformer, box_head, num_queries,
        ...
        self.transformer = transformer   #改
    ...
    def forward_transformer(self, seq_dict, run_box_head=True, run_cls_head=False):
        ...
        # out, outputs_coord = self.forward_box_head(output_embed, enc_mem1, enc_mem2)
        out, outputs_coord = self.forward_box_head(output_embed, enc_mem0, enc_mem5)
        return out, outputs_coord, output_embed

    def forward_box_head(self, hs, memory0, memory5):
        """
        hs: output embeddings (1, B, N, C)
        memory: encoder embeddings (HW1+HW2, B, C)"""
        if self.head_type == "CORNER":
            # adjust shape
            enc_opt0 = memory0[-self.feat_len_s:].transpose(0, 1)  # encoder output for the search region (B, HW, C)
            # enc_opt0_min = enc_opt0.min(dim=2)[0].unsqueeze(2)
            # enc_opt0_max = enc_opt0.max(dim=2)[0].unsqueeze(2)

            # enc_opt0_normed = (enc_opt0 - enc_opt0_min) / (enc_opt0_max - enc_opt0_min)
            # enc_opt0_normed = enc_opt0_normed.transpose(1,2)
            enc_opt0_normed = enc_opt0.sigmoid()  #x超过一定程度，变化较大的时候 y基本不变 

            enc_opt5 = memory5[-self.feat_len_s:].transpose(0, 1)  # encoder output for the search region (B, HW, C)
            enc_opt = enc_opt0_normed * enc_opt5
            # print(enc_opt)+
            dec_opt = hs.squeeze(0).transpose(1, 2)  # (B, C, N)   squeeze(0) 去掉第0维
            # print(dec_opt)
            att = torch.matmul(enc_opt, dec_opt)  # (B, HW, N)
            # print(att)
            opt = (enc_opt.unsqueeze(-1) * att.unsqueeze(-2)).permute((0, 3, 2, 1)).contiguous()  # (B, HW, C, N) --> (B, N, C, HW)
            # print(enc_opt.unsqueeze(-1) * att.unsqueeze(-2))
            bs, Nq, C, HW = opt.size()                              # permute()维度换位   contiguous方法改变了多维数组在内存中的存储顺序，以便配合view方法使用
