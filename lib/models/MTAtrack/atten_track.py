...
class FeatureFusionNetwork(nn.Module):
    ...
    def forward(self, src_temp, mask_temp, src_search, mask_search, pos_temp, pos_search):   #两个模型输入的数据不一样
...
class FeatureFusionLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu"):
        super().__init__()
        self.self_attn1 = nn.MultiheadAttention(d_model, nhead, dropout=dropout)  #total dimension of the model 512  论文上是不是256嘞？？？
        self.self_attn2 = nn.MultiheadAttention(d_model, nhead, dropout=dropout)  #parallel attention heads
        ...
        self.norm11 = nn.LayerNorm(d_model)  #归一化
        ...
        self.activation1 = _get_activation_fn(activation)  #激活层
    ...
    def forward_post(self, src1, src2,
       ...
       #对应Feature future layer的两个ECA
        ...
        #对应Feature future layer的两个CFA
        src12 = self.multihead_attn1(query=self.with_pos_embed(src1, pos_src1), #自己的q
                                   key=self.with_pos_embed(src2, pos_src2), #对方的k v
                                   value=src2, attn_mask=src2_mask,
                                   key_padding_mask=src2_key_padding_mask)[0]
        src22 = self.multihead_attn2(query=self.with_pos_embed(src2, pos_src2), #自己的q
                                   key=self.with_pos_embed(src1, pos_src1), #对方的k v
                                   value=src1, attn_mask=src1_mask,
                                   key_padding_mask=src1_key_padding_mask)[0]

        src1 = src1 + self.dropout12(src12)   #dropout
        src1 = self.norm12(src1)    #norm
        src12 = self.linear12(self.dropout1(self.activation1(self.linear11(src1))))  #再一遍linear activation dropout linear ？
        src1 = src1 + self.dropout13(src12)   #接着dropout norm
        src1 = self.norm13(src1)

        src2 = src2 + self.dropout22(src22)   #同上
