...
class Sequence:
    ...
    def __init__(self, name, frames, dataset, ground_truth_rect, ground_truth_seg=None, init_data=None,
        ...
        self.init_data = self._construct_init_data(init_data)  #第一帧的bbox建立  二维的字典
        self._ensure_start_frame()   #确定开始的第一帧  即第0帧

    def _ensure_start_frame(self):
        # Ensure start frame is 0
        start_frame = min(list(self.init_data.keys()))  #init_data.keys()  返回键 =0
        if start_frame > 0:
            ...
            if self.ground_truth_rect is not None:
                if isinstance(self.ground_truth_rect, (dict, OrderedDict)):
                    ...
                    self.ground_truth_rect = self.ground_truth_rect[start_frame:,:]  #从start_frame开始的数据 这里star_frames=0, 这个条件语句进不来
    ...
    def _construct_init_data(self, init_data):
        if init_data is not None:
            ...
            init_data = {0: dict()}     # Assume start from frame 0  假设从0帧开始
            ...
            if self.ground_truth_rect is not None:
                if self.multiobj_mode:
                    ...
                    if isinstance(self.ground_truth_rect, (dict, OrderedDict)):
                        ...
                        init_data[0]['bbox'] = list(self.ground_truth_rect[0,:])  #取第0集合的全部数据
...
class SequenceList(list):
    """List of sequences. Supports the addition operator to concatenate sequence lists."""   #序列列表 支持加法运算符扩充序列
