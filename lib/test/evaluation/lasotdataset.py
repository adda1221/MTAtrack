...
class LaSOTDataset(BaseDataset):
    ...
    def __init__(self):
        ...
        self.sequence_list = self._get_sequence_list()  #返回测试的数据集的文件名 list
    ...
    def _construct_sequence(self, sequence_name):
        ...
        target_visible = np.logical_and(full_occlusion == 0, out_of_view == 0)  #逻辑与运算 返回布尔值

        frames_path = '{}/{}/{}/img'.format(self.base_path, class_name, sequence_name)

        frames_list = ['{}/{:08d}.jpg'.format(frames_path, frame_number) for frame_number in range(1, ground_truth_rect.shape[0] + 1)]  # 输出矩阵的行数+1

        target_class = class_name
        return Sequence(sequence_name, frames_list, 'lasot', ground_truth_rect.reshape(-1, 4),  #.reshape(-1,4) 转化成4列
