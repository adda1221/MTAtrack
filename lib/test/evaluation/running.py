...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    # else:
    #     try:
    output = tracker.run_sequence(seq, debug=debug)     #debug    default = 0   !!!!   用到了 out = tracker.initialize(image, init_info)
        # except Exception as e:
        #     print(e)
        #     return

    sys.stdout.flush()    #刷新缓冲区  将输出信息直接打印到屏幕上
...
def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8):
    ...
    args:
        ...
        trackers: List of Tracker instances.      #感觉主要是模型
        debug: Debug level.
        threads: Number of threads to use (default 0).
    """
    multiprocessing.set_start_method('spawn', force=True)  #父进程会启动一个全新的 python 解释器进程。 子进程将只继承那些运行进程对象的 run() 方法所必需的资源。 特别地，来自父进程的非必需文件描述符和句柄将不会被继承。 使用此方法启动进程相比使用 fork 或 forkserver 要慢上许多。
